...
export async function runWarmCycle() {
  if (running) {
    console.log('[warm-cron] previous cycle still running; skipping');
    return { skipped: true };
  }
  running = true;
  try {
    ...
    console.log(`[warm-cron] cycle: ${releases.length} releases from ${api}`);
    ...
    for (const e of releases) {
      try {
        ...
        enqueued += m.pending;
        if (m.pending) {
          console.log(`[warm-cron] ${e.owner}/${e.name}@${m.version}: +${m.pending} enqueued (${m.cached}/${m.total} cached)`);
        }
      } catch (err) {
        console.log(`[warm-cron] ${e.owner}/${e.name}: ${err.message}`);
      }
      if (enqueued >= MAX_ENQUEUE) {
        console.log(`[warm-cron] hit cap (${MAX_ENQUEUE} enqueued); resuming next cycle`);
        break;
      }
    }
    console.log(`[warm-cron] cycle done: ${checked} resources checked, ${enqueued} PDFs enqueued`);
    return { checked, enqueued };
  } catch (e) {
    console.log(`[warm-cron] cycle failed: ${e.message}`);
...
export function startWarmCron() {
  ...
  if (!INTERVAL) {
    console.log('[warm-cron] enabled but WARM_INTERVAL_MS not set; not scheduling');
    return;
  }
  console.log(`[warm-cron] every ${INTERVAL}ms, <=${MAX_ENQUEUE} PDFs/cycle, host ${warmApi()}`);
