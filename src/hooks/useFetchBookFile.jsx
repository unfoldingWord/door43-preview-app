...
export default function useFetchBookFile({ catalogEntry, bookId, setErrorMessage }) {
  ...
  useEffect(() => {
    const fetchBookFileContents = async () => {
      ...
      getRepoContentsContent(catalogEntry.repo.url, filePath, catalogEntry.branch_or_tag_name).then((contents) => setFileContents(contents));
    };

    if (catalogEntry && bookId) {
      fetchBookFileContents();
    }
  }, [catalogEntry, bookId, setErrorMessage]);
