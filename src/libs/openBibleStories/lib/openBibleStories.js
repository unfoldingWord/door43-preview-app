...
export const getObsImgZipFileData = async (resolution="360px-compressed") => {
  return await fetch(`https://cdn.door43.org/obs/jpg/obs-images-${resolution}.zip`,
    { cache: "force-cache" }).
...
export async function getOBSData(catalogEntry, zipFileData) {
  ...
  if (catalogEntry.metadata_type == "ts") {
    ...
    if (frontFilePath in zipFileData.files) {
      obsData.front = md.render(
        await zipFileData.file(frontFilePath).async("text")
      );
    } else {
    ...
    if (backFilePath in zipFileData.files) {
      obsData.back = md.render(
        await zipFileData.file(backFilePath).async("text")
      );
    } else {
        ...
        for (let i = 0; i < doc.body.children.length; i++) {
          ...
          if (element.nodeName === "P") {
            if (element.querySelector("img")) {
              ...
              i == doc.body.children.length - 1 &&
              element.childNodes.length === 1 &&
              element.childNodes[0].nodeName === "EM"
            ) {
              story.frames.push(frame);
              frame = null;
...
<article class="obs-front" id="obs-front" data-toc-title="Front">
  ...
  for (const story of obsData.stories) {
    html += `
<section id="obs-${story.storyNum}" class="story" data-toc-title="${encodeHTML(
      story.title
    )}">
...
`;
      if (resolution != "none") {
        html += `
    <img src="${await getOBSImgURL({storyNum: story.storyNum, frameNum: frame.frameNum, imgZipFileData, imgURL: frame.imgURL, resolution})}" alt="Frame ${story.storyNum}-${frame.frameNum}">
`
      }
      html += `
    ${frame.content}
