...
export default function AutocompleteTocNavigation({
  tocSections,
  value,
  onChange,
  sx,
  ...
  useEffect(() => {
    const convertToTocOptions = async () => {
      const toOptions = (section, depth = 0, parentId = null) => {
        ...
        const children = sections.flatMap((child) =>
          toOptions(child, depth + 1, link)
        )
        const option = {
          link,
          title,
      ...
      setTocOptions(tocSections.flatMap(section => 
        toOptions(section)
      ))
    }
