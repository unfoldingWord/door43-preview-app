...
export default function RcBible({
  ...
  useEffect(() => {
    const loadSupportedBooks = async () => {
      ...
      if (!sb.includes(book)) {
        setErrorMessage(
          `Invalid book. \`${book}\` is not an existing book in this resource. Generating for ${
            supportedBooks[0] || "first available book"
          }.`
        );
        return;
      }
      ...
      if (bibleReferenceState?.bookId != book) {
        bibleReferenceActions.goToBookChapterVerse(
          book,
          (book == urlInfo.hashParts[0] && urlInfo.hashParts[1]) || "1",
          (book == urlInfo.hashParts[0] && urlInfo.hashParts[2]) || "1"
        );
      }
    };
  ...
  useEffect(() => {
    const loadUsfmFileFromZipFile = async () => {
      ...
      const text = await zipFileData.files[
        `${catalogEntry.repo.name}/${
          BibleBookData[bibleReferenceState.bookId].usfm
        }.usfm`
      ]?.async("text");
      if (text) {
        setUsfmText(text);
        ...
        setErrorMessage(
          `USFM file for \`${bibleReferenceState.bookId}\` is empty or invalid.`
        );
      }
    };
  ...
  useEffect(() => {
    if (htmlReady && renderedData) {
      let _html = renderedData.replaceAll(
        ...
        `id="${bibleReferenceState.bookId}-$1-$2"`
      );
