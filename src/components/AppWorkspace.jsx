...
export default function AppWorkspace() {
  ...
  const handleDownloadEpub = () => {
    const generateEpub = async (html) => {
      ...
      console.log(html);
      const title = "Open Bible Stories";
      const author = "unfoldingWord";
      ...
      <style>
        ...
        const imgTags = sanitizedContent.match(/<img[^>]+src="([^">]+)"/g);
        if (imgTags) {
          for (const imgTag of imgTags) {
        const imgUrl = imgTag.match(/src="([^">]+)"/)[1];
        const imgName = `image${index}-${imgTags.indexOf(imgTag)}.jpg`;
        console.log("Fetching "+imgUrl);
        const response = await fetch(`https://cors-anywhere.herokuapp.com/${imgUrl}`);
        const blob = await response.blob();
        zip.file(`OEBPS/${imgName}`, blob);
        sanitizedContent = sanitizedContent.replace(imgUrl, imgName);
          }
        }
        const text =
    ...
    generateEpub(htmlSections.body);
  }
  ...
  return (<>
    ...
    <Sheet>
      {!fullScreen && <Header serverInfo={serverInfo} urlInfo={urlInfo} repo={repo} owner={owner} catalogEntry={catalogEntry} bookId={bookId} bookTitle={bookTitle} builtWith={builtWith} onOpenClick={() => setShowSelectResourceModal(!showSelectResourceModal)} />}
